...
class SearchedDoc(BaseDoc):
    retrieved_docs: List[TextDoc] = Field(default_factory=list)
    initial_query: str
    top_n: PositiveInt = 1
...
class RerankedDoc(BaseDoc):
    reranked_docs: List[TextDoc] = Field(default_factory=list)
    initial_query: str
...
class RAGASParams(BaseDoc):
    questions: List[TextDoc] = Field(default_factory=list)
    answers: List[TextDoc] = Field(default_factory=list)
    docs: List[TextDoc] = Field(default_factory=list)
    ground_truths: List[TextDoc] = Field(default_factory=list)
...
class ImagesPath(BaseDoc):
    images_path: List[ImagePath] = Field(default_factory=list)
